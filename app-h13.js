...
const APP_BUILD="20260804-speak7";
...
function pausePlayback(fromRemote){
  ...
  try{ speechSynthesis.cancel(); }catch(e){}
  _mediaHold=true;
  // 必须真正 pause 静音轨：若继续播，系统认为还在播放，单击只会再发 pause，无法恢复
  try{
...
function bindMediaAudioEvents(a){
  ...
  a._srBound=true;
  // 取下耳机时系统常会 pause 静音轨；仅在真正朗读时续上（软暂停不要自动 play）
...
function goPrev(fromRemote){
  ...
  if(wasPlaying) startLoop(rate);
    ...
    try{ speechSynthesis.cancel(); }catch(e){}
    // 软暂停时不要 nudge 播静音轨，否则下一次单击又变成「暂停」
    if(loopPlaying || userWantsPlay) nudgeMediaAudio();
    else if(_mediaHold) setupMediaSession(true);
    render();
    updateMediaSession();
  }
}
...
function setupMediaSession(force){
  ...
  bind("pause", ()=>{
    // 静音轨若仍在播，单击常被系统当成 pause；已软暂停时当作「继续播放」
    if(isSoftPaused()){
      resumeFromRemote();
      return;
    }
