...
const APP_BUILD="20260804-speak6";
...
function startKeepAlives(){
  ...
  speechKeepAlive=setInterval(()=>{
    ...
    // 播放中或暂停挂起时都要续播静音轨，否则取下耳机后再戴上单击无效
    if(userWantsPlay || _mediaHold || loopPlaying){
      nudgeMediaAudio();
      updateMediaSession();
    }
  }, 2500);
...
function pausePlayback(fromRemote){
  ...
  try{ speechSynthesis.cancel(); }catch(e){}
  _mediaHold=true;
  // 关键：不停 mediaKeeper，否则 iOS 丢掉 Now Playing，耳机单击无效
  nudgeMediaAudio();
  // 暂停挂起时仍定期续播静音轨
  startKeepAlives();
...
function bindMediaAudioEvents(a){
  ...
  a._srBound=true;
  // 取下耳机时系统常会 pause 静音轨；会话未结束则尽量续上，避免丢掉 Now Playing
